...
class Chatbot:
    ...
    def reply_at(self, prompt: str, engine_name="stable", sensitivity: int = 1, with_memory: bool = True, fallback_language="english", fallback_empty_string_message="", fallback_no_understanded_message="", fallback_not_sure_message="", temperature: float = 0.0):
        ...
        if engine_name.lower() in ("stable"):
            ...
            last_user_words = set(str(getattr(self, 'last_user_prompt', '')).lower().split()) if with_memory else set()
            last_bot_words = set(str(getattr(self, 'last_bot_output', '')).lower().split()) if with_memory else set()
            ...
            for ex in dataset_list:
                ...
                if not with_memory:
                    ...
                    if has_condition:
                        ...
                        ctx_threshold = sens if sens > 0 else 1.0

                        # Includem în matrice DOAR dacă se trece de pragul de sensibilitate contextuală
                        if last_in_overlap >= ctx_threshold or last_out_overlap >= ctx_threshold:
                            examples.append(ex.get('input_ids', []))
                            outputs.append(ex.get('output_ids', []))
                            examples_meta.append(ex)
                    else:
            ...
            try:
                ...
                try:
                    for i, meta in enumerate(examples_meta):
                        # boost pentru potrivirea memoriei
                        mem_match = 0
                        try:
    ...
    def reply_with_audio(self, prompt, engine_name="stable", sensitivity: int = 1, with_memory: bool = True, fallback_language="english", fallback_empty_string_message="", fallback_no_understanded_message="", fallback_not_sure_message="", temperature: float = 0.0,
