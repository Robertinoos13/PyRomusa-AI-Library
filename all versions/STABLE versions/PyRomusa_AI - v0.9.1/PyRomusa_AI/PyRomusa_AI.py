...
class RealChatbot:
    ...
    class Storage:
        ...
        def load_from_file(self, file_name: str="my_chatbot", file_location: str=""):
            ...
            self.parent.char_to_index        = ckpt["char_to_index"]        # Translator dictionary "text -> token", |dict|
            self.parent.index_to_char        = ckpt["index_to_char"]        # Translator dictionary "token -> text", |dict|
            self.parent.chatbot_name         = ckpt["chatbot_name"]         # A costume name for chatbot to personalize it (no influence the chatbot's performance), |str|
            self.parent.chatbot_description  = ckpt["chatbot_description"]  # A costume info for chatbot to personalize it (no influence the chatbot's performance), |str|
            self.parent.vocabulary           = ckpt["vocabulary"]           # All learned strings by chatbot, |set|
            self.parent.vocabulary_size      = ckpt["vocab_size"]           # The len of "vocabulary" set, |int|
            self.parent.d_model              = ckpt["d_model"]              # Dimensions of the model |int|
            self.parent.num_heads            = ckpt["num_heads"]            # Total attentions heads |int|
            self.parent.num_layers           = ckpt["num_layers"]           # How many times to use the transformers block to generate responses, |int|
            self.parent.max_len              = ckpt["max_len"]              # How long the chatbot can support the prompt (number of tokens), |int|
            self.parent.dropout              = ckpt["dropout"]              # What % of parameters do not use the chatbot when training, |float|
            self.parent.created_on           = ckpt["created_on"]           # When the chatbot was created or when it was saved as a file, |datetime|
            self.parent.final_loss           = ckpt["final_loss"]           # The loss obtained from chatbot training, |float|
            self.parent.total_trained_epochs = ckpt["total_trained_epochs"] # Total trained epoch using bot.trainer.start(), |int|
            self.parent.last_used_datetime   = ckpt["last_used_datetime"]   # When the chatbot was used last time to generate text, |datetime|
            self.parent.tokenizer_type       = ckpt["tokenizer_type"]       # What type of tokenizer does the chatbot understand, |str|
    ...
    class Train:
        ...
        def start(self, epochs: int=100, show_loss_every_x_epochs: int=10, show_loss_process: bool=True, use_gpu_if_is_available: bool=True, learn_late=1e-3, min_learn_rate=1e-5, batch_size=32):
            ...
            # EXTRAGEM VOCABULARUL direct din textul final
            ...
            # ACUM instanțiem LLMModel, știind numărul real de caractere
            self.parent.LLMModel = self.parent._build_llm_model_class()(
                vocab_size=self.parent.vocabulary_size,
                d_model=self.parent.d_model,
                num_heads=self.parent.num_heads,
            ...
            try:
                ...
                if len(training_tensor_data) <= self.parent.block_size:
                    raise ValueError("Your training dataset is too small. Try adding more training examples or adding more text to the current examples and try again.")

                else:
                    raise (f"ERROR: An unexpected error occurred. INFO: {e}")
