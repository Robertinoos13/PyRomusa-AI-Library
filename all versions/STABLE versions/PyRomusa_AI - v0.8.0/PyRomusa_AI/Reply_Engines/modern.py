...
def use_engine(bot_instance, prompt: str, 
                ...
                try:
                    ...
                    for ex in dataset:
                        if prompt_set & ex.get('input_words', set()):
                            ...
                            return format_new_lines(out_str)
            ...
            recent_turns = bot_instance._get_recent_conversation_turns(max_inflenced_memory) if with_memory else []
            if recent_turns:
                if show_thinking == True:
                    ...
                    print(f"This is what I remember as the last conversation:\n     my last message: '{latest_turn.get('bot_output', '')}'\n     user's last message: '{latest_turn.get('user_prompt', '')}'\n")
                    print("Let's start tokenizing the recent user prompts...")

                last_words = []
                for turn in recent_turns:
                    if not isinstance(turn, dict):
                        continue
                    last_words.extend([w for w in str(turn.get('user_prompt', '')).lower().split() if w])
            ...
            if show_thinking:
                print("\nLet's see exactly what else I could extract from the lasts conversation, something useful.")

            # Pregătim variabilele de context dacă with_memory este activ
            last_user_words_list, last_bot_words_list = bot_instance._get_recent_conversation_words(max_inflenced_memory) if with_memory else ([], [])
            ...
            if show_thinking:
                if not last_bot_words and not last_user_words:
                    ...
                    print(bot_instance.conversation_history)
