class Chatbot:
    ...
    class Help:
        ...
        def examples_of_scripts_as_rookie(self):
            return print("-----\n--- Here you will find some scripts examples to practice and know to work with this library. ---\n\nExample 1: The most basic use of this library (to make a functional chatbot)\nfrom muri_ai import Chatbot\n\nbot = Chatbot()\nbot.trainer.add_data('Hello chatbot!', 'Hello human, or what is your name. With what I can help you today?')\nbot.trainer.start()\nbot.reply_at('Hello chatbot!')\n\n\nExample 2: Creating more chatbots in one script, and also naming them\nfrom muri_ai import Chatbot\n\nbot = Chatbot(chatbot_name='Muri1')\nbot1 = Chatbot(chatbot_name='Muri2')\nbot2 = Chatbot(chatbot_name='Muri3')\n-----")
        
        def questions(self):
            return print("-----\n--- Here you will find some questions regarding the possibilities and use of this library, to which you probably wanted to know the answers. ---\n\nQuestion 1: Is possible to create more chatbots in only one script?\nAnswer: Sure, is 100% possible to create more chatbots than one. \nBut how? You need to write more instances (bot = Chatbot(), bot1 = Chatbot(), bot2 = Chatbot(), ...). Also, you can put a name at your chatbot if you want, using the 'chatbot_name' attribute (ex. bot = Chatbot(chatbot_name='Muri Bot 1') ). The names of the chatbots don't affects the performance/comportament of the chatbot's reply/train\n-----")

        def how_to_start(self):
            return print("-----\n\nFirst, we need to know what is this library.\nThis library is a tehnology specialized to create your own chatbot in the simplest mode possible (just 5 lines of code to train your first chatbot: importing library, create chatbot, add an simple input/output example, start the training, generate a response)\n\nWARNING! The logic of this library ('to create an chatbot from 0' concept), is not like an 100% tradicional AI, its maked of own logic of the creator and ONLY INSPIRATED OF THE TRADICIONAL AI FUNCTIONS. For example, no will search things like 'hidden layers', 'different optimizers' or 'loss function' in this library. \n\nBut... \n--- HOW TO START? ---\nHere, was created a function special what works like a helper for this library (already you used that function writing someting like this in your code: bot.helper.how_to_start()). If you want more info, just switch 'how_to_start()' with one of this (depending by your needs):\n- example_of_scripts_as_rookie() - Shows you some complete examples of using this library in your code\n- describe_functions() - A table of some important functions for working with this library and their descriptions\n- questions() - Answers at some questions at what probability you need\n-----")
        
        def describe_functions(self):
            import pandas as pd
            tabel_functions = pd.DataFrame({
                "function": ["bot.trainer.add_data()", "bot.trainer.show_dataset()", "bot.trainer.start()", "bot.trainer.show_translated_examples()", "bot.trainer.show_relationed_output_with_input_words()", "bot.trainer.show_relationed_output_with_input_ids()", "bot.responding_at()", "bot.prepared_datasets.romanian.load_prepared_dataset()", "bot.trainer.show_number_of_examples()"],
            ...
            return print(f"\n-----\n --- Here is described the most important functions of this library (also is tried to be present all functions of all library) in the simplest and cleared mode possible. As rookie, is recomanded to take a look at this functions's info to understand better every function what can do every one --- \n\nNOTE: Before working with this functions of this library, be sure you added this line before starting your work:\nbot = Chatbot()\n\n{tabel_functions}\n-----")
